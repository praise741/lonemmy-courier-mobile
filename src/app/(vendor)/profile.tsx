...
import type { VendorType } from '@/context/AuthContext';
...
export default function VendorProfile() {
  ...
  // Pre-populate form from existing vendor profile
  useEffect(() => {
    if (vendorProfile && !isInitialized) {
      setBusinessName(vendorProfile.businessName || '');
      setAddress(vendorProfile.address || '');
      setDescription(vendorProfile.description || '');
      setTypes(vendorProfile.types || []);
      setLogoUri(vendorProfile.image || null);
      setIsInitialized(true);
    }
  }, [vendorProfile, isInitialized]);
  ...
  const updateMutation = useMutation({
    mutationFn: async (payload: {
      ...
      const res = await api.patch('/vendors/profile', payload);
      return res.data?.data ?? res.data;
    },
    onSuccess: () => {
      showToast('Your vendor profile has been successfully updated.', 'success');
      router.replace('/(vendor)/dashboard');
    },
    onError: (err: any) => {
      showToast(err.response?.data?.message || 'Could not update vendor profile. Please try again.', 'error');
    },
  });
  ...
  // Loading state while vendor profile data hydrates
  if (!isInitialized && vendorProfile) {
    return (
      ...
      >
        ...
        <View style={styles.introSection}>
          ...
          <Text style={[TYPOGRAPHY.body, styles.subtitle]}>
            Update your vendor details visible to JABU students.
          </Text>
        </View>
        ...
        <View style={styles.sectionCard}>
          ...
          {types.length > 0 && (
            <View style={styles.selectedTypesRow}>
              <Text
                style={[TYPOGRAPHY.labelMini, { color: COLORS.secondary }]}
              >
                Selected:{' '}
              </Text>
              {types.map((t) => (
